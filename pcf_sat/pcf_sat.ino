...
PCF8575 minuteExpander(0x20);
PCF8575 sekundeExpander(0x27);

//a=5 b=4 c=7 d=6 gnd=3 LOW=ON tipkalo (prve 4 su tipke zadnja je ground)
const int tipkalo[5]={7,5,6,4,3};

const int sirena = 8;

//a=11 b=9 c=12 d=10 HIGH=ON daljinski
const int daljinski[4]={10,9,12,11};
...
void loop()
{
  ...
  if(pressTime[0] == kratkiDebounce || daljinskiPressTime[0] == daljinskiDebounce){
    if(pause==true){
       zadano = DateTime(0,0,0,zadano.hour(),zadano.minute()+1,zadano.second());
    }
  }
  ...
  if(pressTime[1] == kratkiDebounce || daljinskiPressTime[1] == daljinskiDebounce){
    if(pause==true){
      zadano = DateTime(0,0,0,zadano.hour(),zadano.minute(),zadano.second()+1);
    }
  }
...
void drawTime(TimeSpan time){
  ...
  for (int i = 7; i>=0; i--){
    ((brojevib[time.seconds()/10] >> i)&1) ? sekundeExpander.digitalWrite(7-i, HIGH) : sekundeExpander.digitalWrite(7-i, LOW);
  }

  for (int i = 7; i>=0; i--){
    ((brojevi[time.seconds()%10] >> i)&1) ? sekundeExpander.digitalWrite(15-i, HIGH) : sekundeExpander.digitalWrite(15-i, LOW);
  }

  if (minute <=9){

    for (int i = 7; i>=0; i--){
      ((brojevi[minute%10] >> i)&1) ? minuteExpander.digitalWrite(15-i, HIGH) : minuteExpander.digitalWrite(15-i, LOW);
    }
    ...
    for (int i = 7; i>=0; i--){
      ((brojevib[minute/10] >> i)&1) ? minuteExpander.digitalWrite(7-i, HIGH) : minuteExpander.digitalWrite(7-i, LOW);
    }
